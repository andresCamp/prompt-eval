...
import type { ReveRemixRequest } from '@/components/image-playground/types';

const REVE_ENDPOINT = 'https://api.reve.com/v1/image/remix';

export async function POST(req: Request) {
  const startedAt = Date.now();
  try {
    const body = (await req.json()) as ReveRemixRequest;
    if (!body?.prompt) {
    ...
    if (!response.ok) {
      const message = data?.message || 'REVE remix request failed';
